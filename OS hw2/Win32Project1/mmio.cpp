...
bool
file_copy_using_memory_map_extended(
_In_ const wchar_t* src_file,
_In_ const wchar_t* dst_file
)
{
	...
	if (is_file_existsW(dst_file))
	{
		//print("err ] des file already exists = %ws", dst_file);
		//return false;
		DeleteFileW(dst_file);
	}
	...
	/* src */
	src_ctx->handle = CreateFile(src_file,
	...
	LARGE_INTEGER fileSize;
	if (TRUE != GetFileSizeEx(src_ctx->handle, &fileSize))
	{
		print("err ] GetFileSizeEx( %ws ) failed. gle = %u", src_file, GetLastError());
		ret = false;
		goto clean1;
	}

	src_ctx->map = CreateFileMapping(src_ctx->handle, NULL, PAGE_READONLY, 
		0, 0, NULL);

	if (NULL == src_ctx->map)
	{
		print("err ] CreateFileMapping( %ws ) failed. gle = %u", src_file, GetLastError());
		ret = false;
		goto clean1;
	}

	/* dst */
	dst_ctx->handle = CreateFile(dst_file,
	...
	dst_ctx->map = CreateFileMapping(dst_ctx->handle, NULL, PAGE_READWRITE,
		fileSize.HighPart, fileSize.LowPart, NULL);
	if (NULL == dst_ctx->map)
	{
		print("err ] CreateFileMapping( %ws ) failed. gle = %u", dst_file, GetLastError());
		ret = false;
		goto clean3;
	}

	uint32_t blockSize = 512 * MB;
	uint64_t curOffset = 0;

	/* iterated copy */
	while (curOffset < (uint64_t)fileSize.QuadPart) {
		/* if remaining block is less than blocksize */
		if (blockSize >(uint64_t)fileSize.QuadPart - curOffset) {
			blockSize = (uint64_t)fileSize.QuadPart - curOffset;
		}

		/* src */
		src_ctx->view = (PCHAR)MapViewOfFile(src_ctx->map, FILE_MAP_READ, 0, 0,	blockSize);
		if (NULL == src_ctx->view)
		{
			print("err ] MapViewOfFile( %ws ) failed. gle = %u", src_file, GetLastError());
			ret = false;
			goto clean4;
		}

		/* dst */
		dst_ctx->view = (PCHAR)MapViewOfFile(dst_ctx->map, FILE_MAP_WRITE,
			curOffset >> 32,
			curOffset & 0xffffffff,
			blockSize);
		if (NULL == dst_ctx->view)
		{
		...
		/* copy */
		memcpy(dst_ctx->view, src_ctx->view, blockSize);

		UnmapViewOfFile(dst_ctx->view);
clean5:
		UnmapViewOfFile(src_ctx->view);
		if (!ret) break;

		curOffset += blockSize;
	}
clean4:
	CloseHandle(dst_ctx->map);
clean3:
	CloseHandle(src_ctx->map);
clean2:
	CloseHandle(src_ctx->handle);
clean1:
	CloseHandle(dst_ctx->handle);
